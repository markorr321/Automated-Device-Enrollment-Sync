...
An automated PowerShell tool for synchronizing Apple Business Manager (ADE/DEP) devices with Microsoft Intune. This tool provides an interactive interface with real-time status updates, cooldown timers, and automatic retry functionality.
...
Clone this repository or download the [ADE-Sync.ps1](ADE-Sync.ps1) file:

```powershell
git clone https://github.com/yourusername/Automated-Device-Enrollment-Sync.git
...
1. **Authentication**: You'll be prompted to sign in with your Microsoft 365 credentials
2. **Token Discovery**: The script automatically discovers all configured ADE/DEP tokens in your Intune tenant
3. **Status Display**: For each token, the script displays:
   - Token name and associated Apple ID
   - Last successful sync date/time
   - Last triggered sync date/time
   - Current synced device count
4. **Sync Execution**: The script triggers a sync operation for each token
5. **Cooldown Period**: After each sync, a 15-minute countdown timer begins
6. **Continuous Monitoring**: The process repeats automatically after the cooldown period
...
If you need to modify the required permissions, edit line 112 in [ADE-Sync.ps1](ADE-Sync.ps1:112):
...
- **Solution**: Install the required Microsoft Graph modules using the installation commands above
...
- Built using the [Microsoft Graph PowerShell SDK](https://github.com/microsoftgraph/msgraph-sdk-powershell)
- Designed for integration with [Microsoft Intune](https://intune.microsoft.com) and [Apple Business Manager](https://business.apple.com)
